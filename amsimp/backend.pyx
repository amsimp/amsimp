...
cdef class Backend(Download):
    ...
    at various latitudes of the Earth's surface, under the f-plane
    approximation (2). The unit of measurement is radians per second.
    rossby_parameter ~ generates a NumPy arrray of the Rossby parameter at
    various latitudes of the Earth's surface (2). The unit of measurement is
    radians per second per metre.
    beta_pane ~  generates a NumPy arrray of the Coriolis parameter
    at various latitudes of the Earth's surface, under the beta plane
    approximation (2). The unit of measurement is radians per second.
    ...
    the, atmospheric_prognostic_method, method. Please note that you may specifcy
    whether you would like to save such files throught the utilisation of the
    parameter, remove_files, when the class is initialised.
    ...
    def __cinit__(self, int delta_latitude=5, int delta_longitude=5, bool remove_files=False, forecast_length=72, delta_t=2, bool ai=True, data_size=150, epochs=200, input_date=None, bool input_data=False, geo=None, temp=None, rh=None, u=None, v=None):
        ...
        between 3 and 10 degrees. Defaults to a value of 5 degrees. 
        ...
        updates the data on the Global Data Assimilation System website.
        ...
        # Ensure self.delta_latitude is between 3 and 10 degrees.
        ...
        # at a local sclae.
        if self.delta_latitude > 10 or self.delta_latitude < 3:
            raise Exception(
                "delta_latitude must be a positive integer between 3 and 10. The value of delta_latitude was: {}".format(
                    self.delta_latitude
                )
            )

        # Ensure self.delta_longitude is between 3 and 10 degrees.
        if self.delta_longitude > 10 or self.delta_longitude < 3:
            raise Exception(
                "delta_longitude must be a positive integer between 3 and 10. The value of delta_longitude was: {}".format(
...
# -----------------------------------------------------------------------------------------#
    ...
    cpdef np.ndarray surface_temperature(self):
        """
        This method imports surface temperature data from a GRIB file, which is
        located in the AMSIMP Initial Conditions Data Repository on GitHub.
        The data stored within this repo is updated every six hours by amsimp-bot.
        Following which, it outputs a NumPy array in the shape of
        len(latitude_lines), len(longitude_lines)).

        Surface Temperature is defined as the mean kinetic energy density of molecular
        ...
        if not self.input_data:
            ...
            # Ensure that the correct data was downloaded (surface temperature).
            if temp_cube[3].units != units.K:
                raise Exception("Unable to determine the surface temperature"
                + " at this time. Please contact the developer for futher"
                + " assistance.")
            elif temp_cube[3].metadata[0] != 'surface_temperature':
                raise Exception("Unable to determine the surface temperature"
                + " at this time. Please contact the developer for futher"
                + " assistance.")
        else:
        ...
        if np.shape(temp) == (181, 360):
            ...
            surface_temperature = np.concatenate((nh_temp, sh_temp))

            # Reshape the data in such a way that it matches the longitude lines defined in
            # the software.
            surface_temperature = surface_temperature[::-1, ::self.delta_longitude]
            
            if self.remove_files and not self.input_data:
                os.remove("initial_conditions.nc")

            # Define the unit of measurement for surface temperature.
            surface_temperature *= units.K
        else:
            surface_temperature = temp * units.K
...
# -----------------------------------------------------------------------------------------#
    ...
    cpdef np.ndarray gravitational_acceleration(self):
        ...
        FGravitational acceleration is the free fall acceleration of an object
...
# -----------------------------------------------------------------------------------------#
    ...
    def longitude_contourf(self, which=0, psurface=1000):
        ...
        For a surface temperature contour plot, the value of which is 0.
        For a temperature contour plot, the value of which is 1.
        For a geopotential height contour plot, the value of which is 2.
        For a density contour plot, the value of which is 3.
        For a relative humidity, the value of which is 4.
        For a virtual temperature contour plot, the value of which is 5.
        For a vapor pressure contour plot, the value of which is 6.
        For a potential temperature contour plot, the value of which is 7.
        """
        info = " For a surface temperature contour plot, the value of which is 0. "
        info += " For a temperature contour plot, the value of which is 1. "
        info += "For a geopotential height contour plot, the value of which is 2. For a "
        info += "density contour plot, the value of which is 3. For a relative humidity "
