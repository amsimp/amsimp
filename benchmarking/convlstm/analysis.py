...
# AMSIMP.
# 2 metre temperature.
t2m = np.load(folder + "t2m.npy")
amsimp_t2m, naive_t2m = np.split(t2m, 2, axis=1)
# Geopotential.
...
# 2 metre temperature
comparsion_t2m = np.load(folder + "2m_temperature.npy")
...
for i in range(3):
    ...
    plot(
        x1, x2, amsimp_t2m[:, i, :], naive_t2m[:, i, :], comparsion_t2m[i, :, :], title, metric
    )
