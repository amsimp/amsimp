...
# Air temperature.
input_temperature, obs_temperature = preprocess_data(temperature, (15*12), (30*12))
input_temperature, obs_temperature = input_temperature[::(10*12)], obs_temperature[::(10*12)]
t.update(1)
# Relative humidity.
input_relativehumidity, obs_relativehumidity = preprocess_data(relative_humidity, (15*12), (30*12))
input_relativehumidity, obs_relativehumidity = input_relativehumidity[::(10*12)], obs_relativehumidity[::(10*12)]
t.update(1)
# Geopotential.
input_geopotential, obs_geopotential = preprocess_data(geopotential, (15*12), (30*12))
input_geopotential, obs_geopotential = input_geopotential[::(10*12)], obs_geopotential[::(10*12)]
t.update(1)
# Wind.
# Zonal.
input_zonalwind, obs_zonalwind = preprocess_data(zonal_wind, (15*12), (30*12))
input_zonalwind, obs_zonalwind = input_zonalwind[::(10*12)], obs_zonalwind[::(10*12)]
t.update(1)
# Meridional.
input_meridionalwind, obs_meridionalwind = preprocess_data(meridional_wind, (15*12), (30*12))
...
accuracy_temperature = np.zeros((len_test, 6, (30*12)))
level_800 = iris.Constraint(pressure_level=800)
# Geopotential at 500hPa.
accuracy_geopotential = np.zeros((len_test, 6, (30*12)))
...
for i in range(len_test):
    ...
    state = amsimp.Weather(historical_data=model_input, forecast_length=720)
...
# Save results.
# Air temperature.
np.save('results/climate-temperature.npy', accuracy_temperature)
# Geopotential.
np.save('results/climate-geopotential.npy', accuracy_geopotential)
