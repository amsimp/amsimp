...
# Load data.
historical_data = iris.load("*.nc")

# Define constrainits.
# Air temperature at 850 hPa.
level_850 = iris.Constraint(pressure_level=850)
...
# Air temperature.
t = historical_data.extract("t")[0].extract(level_850)
# Geopotential.
z = historical_data.extract("z")[0].extract(level_500)
...
for i in range(len_test):
    ...
    model_input = CubeList([input_t[i].copy(), input_z[i].copy()])
    ...
    fct_t = fct_t.extract(level_850)

    # Geopotential at 500 hPa.
    fct_z = fct.extract("z")[0]
    fct_z = fct_z.extract(level_500)
